...
const PageTitleInner = styled.div<PageTitleProps>`
    ...
    p {
        ...
        text-align: center;
        font-size: ${props =>
            props.subTitle ? '1.6rem' : props.title ? '2.6rem' : '1.6rem'};
        margin: ${props =>
            props.subTitle ? '0.3rem' : props.title ? '0' : '0'};
        /* padding:1rem 0 0 0; */
...
const PageTitle: React.FunctionComponent<PageTitleProps> = ({
    ...
    const _renderTextContent = (arg: string | string[]) => {
        if (typeof arg === 'object') {
            ...
            return arrayOfArg.length && arrayOfArg.length > 1
                ? arrayOfArg.map(p => <p>{p}</p>)
                : null
    ...
    return (
        <PageTitleContainer>
            <PageTitleInner title={title} subTitle={subTitle}>
